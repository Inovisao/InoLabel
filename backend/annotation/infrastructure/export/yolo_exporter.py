"""Exportadores YOLO e escrita de arquivos."""
...
from backend.annotation.core.augmentation.augmentation_service import apply_preset
from backend.annotation.core.augmentation.augmentation_types import AugmentationPreset
from backend.annotation.core.export.split_service import assign_splits
from backend.annotation.core.export.yolo_label_service import (
...
def _write_augmented_copies(
    ...
    if image is None:
        malformed_labels.append(f"{target_image_path.name}: falha ao ler imagem para augmentation")
        return 0, 0
    ...
    for idx, (aug_image, aug_boxes) in enumerate(apply_preset(image, yolo_boxes, augmentation_preset)):
        ...
        if not cv2.imwrite(str(aug_image_path), aug_image):
            malformed_labels.append(f"{aug_image_path.name}: falha ao salvar imagem augmentada")
            continue
