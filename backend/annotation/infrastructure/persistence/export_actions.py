...
from backend.dataset_export import export_detection_coco_json, export_yolo_dataset, export_yolo_no_split, load_json
...
class ExportActionsMixin:
    ...
    def load_export_payload_from_state(self) -> dict:
        # Salva frame atual antes de exportar para garantir consistencia
        if hasattr(self, "autosave_current_frame"):
            self.autosave_current_frame(reason="exportar dataset")
        if hasattr(self, "write_annotations"):
