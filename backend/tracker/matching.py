...
try:
    ...
    _USE_CYTHON_BBOX = True
except ImportError:
    _USE_CYTHON_BBOX = False

from tracker import kalman_filter


def _numpy_bbox_ious(atlbrs, btlbrs):
    atlbrs = np.array(atlbrs, dtype=float)
    btlbrs = np.array(btlbrs, dtype=float)
    area_a = (atlbrs[:, 2] - atlbrs[:, 0]) * (atlbrs[:, 3] - atlbrs[:, 1])
    area_b = (btlbrs[:, 2] - btlbrs[:, 0]) * (btlbrs[:, 3] - btlbrs[:, 1])
    inter_x1 = np.maximum(atlbrs[:, None, 0], btlbrs[None, :, 0])
    inter_y1 = np.maximum(atlbrs[:, None, 1], btlbrs[None, :, 1])
    inter_x2 = np.minimum(atlbrs[:, None, 2], btlbrs[None, :, 2])
    inter_y2 = np.minimum(atlbrs[:, None, 3], btlbrs[None, :, 3])
...
def bbox_ious(atlbrs, btlbrs):
    if _USE_CYTHON_BBOX:
        return _cython_bbox_ious(
            np.ascontiguousarray(atlbrs, dtype=float),
            np.ascontiguousarray(btlbrs, dtype=float)
        )
    return _numpy_bbox_ious(atlbrs, btlbrs)
