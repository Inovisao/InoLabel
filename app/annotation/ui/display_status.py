...
class DisplayStatusMixin:
    ...
    def current_review_record(self) -> Optional[dict]:
        """Returns the record currently in review."""
    ...
    def current_deletable_image_path(self) -> Optional[Path]:
        """Returns the current image that can be deleted from the UI button."""
    ...
    def build_status_message(self) -> str:
        """Short message for the top bar — active mode and current source."""
    ...
    def update_status(self):
        """Updates the top bar, status blocks and buttons."""
    ...
    def update_status_blocks(self):
        """Populates the five status-strip blocks with structured state."""
        if not hasattr(self, "status_source_var"):
            return

        # Block 1: source + frame
    ...
    def current_display_file_name(self) -> str:
        """User-friendly name of the current image/frame."""
        record = self.current_review_record()
        if record is not None:
    ...
    def current_open_target_path(self) -> Optional[Path]:
        """Returns the path of the current image that can be opened in the file manager."""
        record = self.current_review_record()
        if record is not None:
    ...
    def update_image_info(self):
        """Updates the image name label and the state of the 'Show in folder' button."""
    ...
    def open_in_file_manager(self, target: Path) -> bool:
        """Opens the file manager and attempts to highlight the target file."""
        return reveal_path(target)

    def on_open_in_folder(self):
        """Button event to open the current image in the file manager."""
        target = self.current_open_target_path()
